...
if __name__ == "__main__":
    ...
    if mnist:
        ...
        # plot the 10 images in a single plot
        fig, axs = plt.subplots(2, 5)
        for i in range(10):
            axs[i//5][i%5].imshow(data_X[i], cmap='gray')
            axs[i//5][i%5].axis('off')
            axs[i//5][i%5].text(0.8, -1, str(i), fontsize=12, color='white')
        plt.subplots_adjust(wspace=0.5)
        plt.suptitle('MNIST average images', fontsize=16, color='white')
        # set background color to black
        fig.patch.set_facecolor('black')
        plt.savefig('./mnist_average_images.png')
        plt.show()
